    ...
    def test_media_approval(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            try:
                ...
                logging.info("Successfully Clicked on the done button - Testcase 1.7 is passed")
        ...
        with caplog.at_level(logging.INFO):
            try:
                ...
                time.sleep(3)
                logging.info("Successfully logged in to the admin panel - Testcase 1.13 is passed")

            except (NoSuchElementException, ElementNotInteractableException):
                logging.error("Element not found:{e}")

        """"Clicking on the projects link"""
        with caplog.at_level(logging.INFO):
            try:
                logging.info("Clicking on the projects link ")
                project_button = WebDriverWait(self.driver, 10).until(BD.element_to_be_clickable(
                    (By.XPATH, "//p[text()='Projects']")))
        ...
        """"Clicking on the projects to approve"""
        with caplog.at_level(logging.INFO):
            try:
                logging.info("Clicking on the projects to approve ")
                specific_project = WebDriverWait(self.driver, 10).until(BD.element_to_be_clickable(
                    (By.XPATH, "//p[@title='Aims hospital']")))
        ...
        with caplog.at_level(logging.INFO):
            try:
                ...
                id_button.send_keys("AIMS-project011")
                time.sleep(3)
        ...
        with caplog.at_level(logging.INFO):
            try:
                ...
                logging.info("Successfully navigated - Testcase 1.22 is passed")
        ...
        with caplog.at_level(logging.INFO):
            try:
                ...
                keyboard = Controller()
                time.sleep(1)
                keyboard.type(r"C:\Users\muduu\Downloads\project2.jpg")
        ...
        with caplog.at_level(logging.INFO):
            try:
                ...
                logging.info("Successfully Clicked on the done button - Testcase 1.26 is passed")
        ...
        """Scroll down till the page visible"""
        element = WebDriverWait(self.driver, 10).until(
            BD.element_to_be_clickable((By.XPATH, "(//label[text()='Client Name'])[1]")))
        self.driver.execute_script("arguments[0].scrollIntoView(true);", element)
        time.sleep(3)
        ...
        with caplog.at_level(logging.INFO):
            try:
                ...
                approved_button = WebDriverWait(self.driver, 10).until(BD.element_to_be_clickable(
                    (By.CSS_SELECTOR, ".bg-blue-500.border.border-blue-500.rounded-full.gap-2.flex.items-center.p-2")))
